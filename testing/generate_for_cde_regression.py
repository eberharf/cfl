...
def get_params(): 
    params = { 'show_plot' : False,
               'n_epochs' : 100,  # longer training time to try to get consistent results 
            } 
    return params
...
def generate_correct_results(): 
    ...
    for cde in cde_input_shapes.keys():
        ...
        # flatten X if needed 
        if cde_input_shapes[cde] < len(X.shape): # assuming that X will stay 3-d (n_images, im_height, im_width) 
            # and that the only desired CDE input dimensionality smaller than
            # 3-d would be 2-d 
            X = np.reshape(X, (X.shape[0], X.shape[1] * X.shape[2]))
        
        # expand X if needed: 
        if cde_input_shapes[cde] > len(X.shape): # the case where X is 3-d but we need it 4-d (n_images, im_height, im_width, n_channels)
