...
def compute_predictive_error(Xlr, Ylr):
    ''' 
    Fits and evaluates a linear model to 10 folds of data, then returns
    the average error.
    ...
    errs = -cross_val_score(clf, Xlr, Ylr, cv=10,
                            scoring='neg_mean_squared_error')
    return np.nanmean(errs), np.nanstd(errs)
...
def visualize_errors(errs, stds, params_list, params_to_tune, vis_thresh=1e4):
    '''
    ...
    Arguments:
        ...
        stds (np.ndarray) : array of standard deviation for every parameter combination
    ...
    if len(tuned_k) < 1:
        ...
        np.save('clusterer_grid_search.npy', shaped_errs)

        # mask any outliers
        shaped_errs = np.ma.masked_where(
            shaped_errs > vis_thresh, shaped_errs)
        shaped_errs[shaped_errs > vis_thresh] = np.nan
        ...
        ax.fill_between(params_to_tune[k0], shaped_errs-stds, shaped_errs+stds, alpha=0.5)
        ax.set_xlim([params_to_tune[k0][0], params_to_tune[k0][-1]])
        ax.set_xlabel(k0)
        ax.set_ylabel('MSE')
        plt.tight_layout()
...
def tune(data_to_cluster, model_name, model_params, user_input):
    ...
    stds = np.zeros((len(param_combos),))
    ...
    for ci, cur_params in tqdm(enumerate(param_combos.copy())):
        ...
        lbls = tmp_model.fit_predict(data_to_cluster)
        good_clusters = True
        if model_name=='KMeans':
            # check if any clusters have no members
            n_clusters = cur_params['n_clusters']
            n_members = [np.sum(lbls==i) for i in range(n_clusters)]
            if np.min(n_members) < 1:
                good_clusters = False
        
        if good_clusters:
            # compute error measure (error in predicting pyx from Xbarhat)
            errs[ci], stds[ci] = compute_predictive_error(
    ...
    fig = visualize_errors(errs, stds, param_combos, model_params)
    try:
        suggested_params = param_combos[suggest_elbow_idx(errs)]
    except:
        suggested_params = param_combos[np.nanargmin(errs)]
