...
def _kl_divergence(p, q):
    ...
    kl_pq = np.sum(p * np.log2(np.divide(p, q, out=np.zeros_like(p), where=q!=0)))
    kl_qp = np.sum(q * np.log2(np.divide(q, p, out=np.zeros_like(q), where=p!=0)))
        ...
        for cj in range(n_clusters):
            ...
            if (ci_samples.shape[0] > 0) and (cj_samples.shape[0] > 0):
                for f in range(n_features):
                    disc_vals[ci,cj,f] = disc_func(ci_samples[:,f], cj_samples[:,f])
            else:
